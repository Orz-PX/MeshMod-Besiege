...
public class MeshBlockScript : BlockScript
{
    ...
    public override void SafeAwake()
    {
        ...
        MeshCollider AddMeshCollider()
        {
            Transform tra = GetComponentsInChildren<Transform>().ToList().Find(match => match.name == "Colliders");
            Debug.Log(tra.name);
            GameObject gameObject;
            if (tra.GetComponentInChildren<MeshCollider>() == null)
            {
                gameObject = new GameObject("Mesh Collider");
                gameObject.transform.SetParent(tra);
            }
            else
            {
